...
from src.auth_service.app.crud.user import get_user
from src.auth_service.app.models.user import AuthUser
from src.auth_service.app.schemas.token import Token
from src.auth_service.app.utils.email import send_verification_email, send_password_reset_email
from src.auth_service.app.utils.password import get_password_hash
from src.auth_service.app.utils.security import create_access_token, create_refresh_token, generate_confirmation_code, \
    generate_reset_code
...
async def register_user(db: AsyncSession, email: EmailStr, password: str) -> dict:
    ...
    """
    # Check user exists
    user = await db.execute(select(AuthUser).filter(AuthUser.email == email))
    ...
    # Creating new user
    new_user = AuthUser(email=email, hashed_password=hashed_password, confirmation_code=confirmation_code)
    db.add(new_user)
    await db.commit()
...
async def logout_user(db: AsyncSession, user: AuthUser) -> dict:
    """
    Logout user.
    """
    # Clear Refresh Token
